...
\item
\begin{enumerate}[(a)]
\item Let $G^1$ and $G^2$ be a partnership game and congestion game respectively and suppose that they have the same number of players $k$. For the bijection $f_i$ to exists from the strategies $A_i$ of player $i$ in $G^1$ to the strategies $B_i$ of player $i$ in $G^2$, we require $|A_i|=|B_i|$ for all $i\in \{1,\ldots,k\}$.
