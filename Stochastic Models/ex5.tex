...
We shall prove the claim that $\sum^{k}_{j=0}P^n_{ij}=1$. It is easy to see that it holds for $n=1$. Suppose that it is true for $n$, then since we have
\begin{align*}
\sum^{k}_{j=0}P^{n+1}_{ij} = \sum^{k}_{j=0}\sum_{l=0}^{k}P^{n}_{il}P_{lj}= \sum_{l=0}^{k}P^{n}_{il}\sum^{k}_{j=0}P_{lj}
\end{align*}
