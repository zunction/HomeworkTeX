...
%	An agent $i$ chooses to route its resources, time, $\mathbf{t_i}=(t_{i1},t_{i2})$ for $i=1,2$ to the different projects available where $t_{ii}$ is the time devoted to agent $i$'s own project and $t_{ij}$ is the time routed to agent $j$'s project. We would like to adopt the idea of a production function that was used by \cite{Drago1991}
%	\begin{align*}
%	
%	\end{align*}
%	
