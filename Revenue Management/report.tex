...
\end{align}
as we have $\sum_{i=1}^{n}P_t^i + P_t^0 = 1$, it suffices to find $P_t^i$ for $i>0$. Without going into details, we see that the maximizer has the form
\begin{align}
...
\begin{align}
\left[\frac{m_t(c_t)}{\mu}-1\right] \exp \left[\frac{m_t(c_t)+u_0}{\mu}\right] = \sum_{i=1}^{n}\exp \left[\frac{q^i-\Delta^i \hat{\mathbf{w}}_t^{\mathbf{P}^\ast}(c_t)}{\mu}\right]
\end{align}
where $m_t(c_t)$ is the unique solution. This uniqueness follows from the concave property of $\Psi_t^{\mathbf{P}}(c_t)$ over probability simplex $P_t$. Using the results that we have derived above, the optimal price scheme is
\begin{align}
...
\begin{align}
\frac{\partial}{\partial t}\hat{\mathbf{w}}_t^{\mathbf{P}^\ast} = \lambda(t)(m_t(c_t) - \mu)
\end{align}
